...
const BerriesMaker = artifacts.require('BerriesMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('BerriesMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.berry = await BerryToken.new({ from: alice });
        await this.berry.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await BerriesMaker.new(this.factory.address, bar, this.berry.address, this.weth.address);
        this.sushiWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.berry.address)).logs[0].args.pair);
    ...
    it('should make BERRYs successfully', async () => {
        ...
        await this.berry.transfer(this.sushiWETH.address, '10000000', { from: minter });
        ...
        // After calling convert, bar should have BERRY value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.berry.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for BERRY-ETH pair
        await this.berry.transfer(this.sushiWETH.address, '100000', { from: minter });
        ...
        await this.maker.convert(this.berry.address, this.weth.address);
        assert.equal((await this.berry.balanceOf(bar)).valueOf(), '66249');
