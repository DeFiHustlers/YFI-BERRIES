...
contract BerriesMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _berry, address _weth) public {
        factory = _factory;
        berry = _berry;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            IERC20(token).transfer(factory.getPair(weth, berry), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, berry), new bytes(0));
        return amountOut;
    }

    function _toBERRY(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, berry));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
